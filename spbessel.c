...
complex long double dspbesj0l (complex long double);
complex long double dspbesh0l (complex long double);
void spbesjinit (complex double *, complex double, int);
void spbeshinit (complex double *, complex double, int);
void spbesjinitl (complex long double *, complex long double, int);
void spbeshinitl (complex long double *, complex long double, int);
...
int spbeshl (complex long double *vals, complex long double z, int n) {
	int i, nmo;

	if (n < 0) return -1;

	/* Build the first orders of the Hankel function. */
	spbeshinitl (vals, z, n);

	/* Recursion fills out the rest of the orders. */
	nmo = n - 1;
	for (i = 1; i < nmo; ++i)
		vals[i+1] = (2L * (long double)i + 1L) * vals[i] / z - vals[i - 1];
...
int spbesjl (complex long double *vals, complex long double z, int n) {
	int i, nmo;

	if (n < 0) return -1;

	/* Build the first orders of the Hankel function. */
	spbesjinitl (vals, z, n);
...
void spbesjinitl (complex long double *jf, complex long double x, int nmax) {
	jf[0] = csinl(x) / x;
...
void spbeshinitl (complex long double *hf, complex long double x, int nmax) {
	complex long double cxfact;

	cxfact = -cexpl (1iL * x) / x;
...
complex long double dspbesh0l (complex long double x) {
	complex long double fact;

	fact = (x + 1iL) / (x * x);

	return fact * cexpl (1iL * x);
}
...
complex long double dspbesj0l (complex long double x) {
	complex long double sx, cx;
...
int dspbeshl (complex long double *vals, complex long double *hfn,
		complex long double z, int n) {
	int i;
	 ...
	 * derivative. */
	for (i = 1; i < n; ++i) 
		vals[i] = 0.5L * (hfn[i - 1] - hfn[i + 1]) - hfn[i] / (2.0L * z);
...
int dspbesjl (complex long double *vals, complex long double *hfn,
		complex long double z, int n) {
	int i;
